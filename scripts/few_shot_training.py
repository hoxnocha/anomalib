...
def train_step(
    ...
    elif config.dataset.name == "airogs":
        config.dataset.format = "airogs"
    else:
...
def get_args(raw_args=None) -> Namespace:
    ...
    parser.add_argument(
        "--backbone-config",
        type=str,
        default="/home/students/tyang/anomalib/src/anomalib/config/backbone_config.yaml",
...
def train(raw_args=None):
    ...
    if config.dataset.name == "mvtec":
        ...
        from anomalib.data.airogs import AIROGS_CATEGORIES
        categories = AIROGS_CATEGORIES
    else:
